...
6. **A published mapping between a value and its standards key.** Building the
...
Next: contract tests against a running mechanica release, so the boundary is
checked by something that runs rather than by the viewer looking right.
