...
stage cannot be assembled without, one standards table they all need, and the
frames fabrica has already committed to in `stubs/` so that a built part and a
stub agree rather than nearly agreeing. It opens with a fault in the stage
itself, because a bracket designed against the current machine file would bake
it into hardware.

Below is the general list - the shape of what a machine needs from a part
catalogue, rather than which parts.
